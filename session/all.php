<html>
  ...
  1		00% {background-color:red; left:0px; top:0px;}
}
</style>
  </head>
  <body>
   ...
   <?php
    //Rozpoczęcie sesji i jej sprawdzenie, aby możliwe było pobranie danych z JavaScript
	session_start();
	...
	$sql = "INSERT INTO goscieportalu (ipaddress, datetime, browser, resolution, screen, colors, cookies, java, language) VALUES ('$ipaddress', CURRENT_TIMESTAMP,'$matches[0]','$resolution','$screen','$colors','$cookies','$java','$language');";
    ...
    "> 
       ...
       if ($result = $mysqli->query($query)) {
        ...
        $f1 = $row["datetime"];
      	$f2 = $row["ipaddress"];
      	$f3 = $details_ip -> country.", ". $details_ip -> region.", ".$details_ip -> city;
        $f4 = $details_ip -> loc;
    ...
    $result->free();
		...
		echo "Sprawdzam sesję!";
  		header('Refresh: 1; URL=all.php?dir=../users/$user1');
  		exit();
}
